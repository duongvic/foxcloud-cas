...
import foxcloud
from foxcloud import exceptions as fox_exec
...
_type_error_msg = _("'%(other)s' should be an shade of '%(cls)s'")
    ...
    def __init__(self, version_str=None):
        ...
        if version_str is not None:
            ...
            if match:
                ...
                raise fox_exec.FoxCloudUnSupportedVersion(msg)
...
def check_major_version(api_version):
    ...
    :raises fox_exec.: if major part is not
                                                      supported
    """
...
def discover_version(client, requested_version):
    ...
    if (not requested_version.is_latest() and
        ...
        if server_start_version.is_null() and server_end_version.is_null():
            raise fox_exec.FoxCloudUnSupportedVersion(
    ...
    if server_start_version.is_null() and server_end_version.is_null():
        return APIVersion('2.0')
    elif foxcloud.API_MIN_VERSION > server_end_version:
        raise fox_exec.FoxCloudUnSupportedVersion(
            _("Server version is too old. The client valid version range is "
              ...
              "range is '%(server_min)s' to '%(server_max)s'.") % {
                  'client_min': foxcloud.API_MIN_VERSION.get_string(),
                  'client_max': foxcloud.API_MAX_VERSION.get_string(),
                  'server_min': server_start_version.get_string(),
                  'server_max': server_end_version.get_string()})
    elif foxcloud.API_MAX_VERSION < server_start_version:
        raise fox_exec.FoxCloudUnSupportedVersion(
            _("Server version is too new. The client valid version range is "
              ...
              "range is '%(server_min)s' to '%(server_max)s'.") % {
                  ...
                  'server_min': server_start_version.get_string(),
                  'server_max': server_end_version.get_string()})
    elif foxcloud.API_MAX_VERSION <= server_end_version:
        return foxcloud.API_MAX_VERSION
    elif server_end_version < foxcloud.API_MAX_VERSION:
...
def wraps(start_version, end_version=None):
    ...
    def decor(func):
        ...
        def substitution(obj, *args, **kwargs):
            ...
            if not methods:
                raise fox_exec.FoxCloudVersionNotFoundForAPIMethod(
