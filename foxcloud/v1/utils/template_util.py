...
from foxcloud.v1.utils import template_format
from foxcloud.v1.utils import environment_format
...
def get_template_contents(template_file=None, template_url=None,
    ...
    if template_file:
        template_url = sys_util.normalise_file_path_to_url(template_file)
    ...
    tmpl_base_url = sys_util.base_url_for_url(template_url)
    if files is None:
        files = {}
        ...
        for key, value in from_data.items():
            ...
            if str_url not in files:
                if is_object and object_request:
                    ...
                    file_content = sys_util.read_url_content(str_url)
    ...
    if env_paths:
        for env_path in env_paths:
            ...
            if env_list_tracker is not None:
                env_url = sys_util.normalise_file_path_to_url(env_path)
...
def process_environment_and_files(env_path=None,
    ...
    if is_object:
        ...
        env_base_url = sys_util.base_url_for_url(env_path)
        ...
        env_url = sys_util.normalise_file_path_to_url(env_path)
        env_base_url = sys_util.base_url_for_url(env_url)
...
def resolve_environment_urls(resource_registry, files, env_base_url,
    ...
    :type  files: dict
    :param env_base_url: stack URL to look in when loading files
