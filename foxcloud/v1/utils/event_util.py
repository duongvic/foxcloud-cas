...
def get_events(cloud, stack_id, event_args, marker=None, limit=None):
    ...
    data = cloud.events.list(stack_id=stack_id,
                             resource_name=None, **event_args)
    events = meta.get_and_munchify(None, data)
...
def poll_for_events(
    ...
    while True:
        ...
        if no_event_polls >= 2:
            ...
            stack = cloud.stacks.get(stack_id=stack_name, resolve_outputs=False)
